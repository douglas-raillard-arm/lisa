...
mod private {
        ...
        <Feat as Feature>::Config: Debug + for<'a> Deserialize<'a> + Send + Clone,
    {
        ...
        fn __start(
            ...
            match config.get::<Self>() {
                Some(DependencySpec::Mandatory { .. }) | None => start(),
                Some(DependencySpec::Optional { .. }) => match start() {
                    Ok(()) => Ok(()),
                    Err(err) => {
                        pr_info!(
                ...
                Some(DependencySpec::Disabled) => Ok(()),
            }
        }
            ...
            match cursor.value() {
                Selected::Selected(live) => {
                    let mut start_children = |children_service: &mut _| {
                        cursor
                            .children()
                            .try_for_each(|child_cursor| {
                                let child_feature = match cursor.value() {
                                    Selected::Selected(feature) => &feature.feature,
                                    Selected::NotSelected(feature) => feature,
                                };
                                process(Some(live), &child_cursor, children_service)
                            })
                            .with_context(|| {
                                format!(
                    ...
                    live.feature
                        .__start(live, parent, config, parents_service, &mut start_children)
                }
    ...
    for config in configs {
        for (feat_name, feat_config) in config {
            match features_by_name.get(&*feat_name) {
                ...
                Some(feat) => {
                    feat.__push_user_config(stack, feat_config)?;
                    Ok(())
                }
            }?;
        }
    ...
    Select: Fn(&dyn Feature) -> bool,
{
    ...
    let graph = graph.dfs_map(DfsPostTraversal::new(
        ...
        |value: Arc<dyn Feature>, mut parents: &mut dyn Iterator<Item = &private::Selected<_>>| {
            ...
            // selected.
            let selected = (&mut parents)
                .any(|parent| matches!(parent, private::Selected::Selected(_)))
                || select(&*value);
            if selected {
                private::Selected::Selected(value)
            } else {
    ...
    >| {
        match feature {
            private::Selected::Selected(feature) => feature
                .__configure(parents, &stacked_configs)
                .map(private::Selected::Selected),
