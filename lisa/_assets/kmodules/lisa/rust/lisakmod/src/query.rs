...
use crate::{
    ...
    features::{
        DependenciesSpec, DependencySpec, Feature, GenericConfig, Visibility, features_lifecycle,
        ...
        match self {
            ...
            Query::StartFeatures => {
                ...
                let select = |feature: &dyn Feature| match feature.name() {
                    name if features.contains(name) => {
                        #[allow(clippy::if_same_then_else)]
                        if feature.visibility() == Visibility::Public {
                            true
                        } else if name == LegacyFeatures::NAME {
                            true
                        } else {
                            pr_err!("Feature {name} cannot be explicitly enabled as it is private");
                            false
                        }
                    }
                    _ => false,
