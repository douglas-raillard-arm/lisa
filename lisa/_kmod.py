...
from lisa.utils import nullcontext, Loggable, LISA_CACHE_HOME, checksum, DirCache, chain_cm, memoized, LISA_HOST_ABI, subprocess_log
...
def _make_chroot(make_vars, bind_paths=None, alpine_version='3.14.2', overlay_backend=None):
    ...
    chroot_arch = make_vars.get('ARCH', LISA_HOST_ABI)
...
@contextlib.contextmanager
def _overlay_folders(lowers, upper=None, backend=None):
    """
...
class KernelTree(Loggable):
    ...
    @classmethod
    def _prepare_tree(cls, path, make_vars, build_env):
        logger = cls.get_logger()
        _make_vars = [
            f'{name}={val}'
        ...
        make_targets = ['olddefconfig', 'modules_prepare']
        ...
        if build_env != 'host':
            make_targets = ['clean'] + make_targets

        cmd = ['make', f'-j{nr_cpus}', '-C', str(path), '--', *_make_vars, *make_targets]
        ...
        if build_env == 'alpine':
            ...
            def cmd_cm():
                with _make_chroot(bind_paths=bind_paths, make_vars=make_vars) as chroot:
                    yield _make_chroot_cmd(chroot, cmd)
        else:
            cmd_cm = lambda: nullcontext(cmd)

        with cmd_cm() as _cmd:
            pretty_cmd = ' '.join(map(str, _cmd))
            logger.info(f'Preparing kernel tree for modules')
            subprocess_log(_cmd, logger=logger, level=logging.DEBUG)
    ...
    def from_overlays(cls, version=None, tree_path=None, overlays=None, make_vars=None, cache=True, build_env=None, overlay_backend=None):
        ...
        def prepare_overlay(path):
            ...
            apply_overlays(path)
            cls._prepare_tree(path, make_vars=make_vars, build_env=build_env)
            # Re-apply the overlays, since prepare_tree() could have
            # overwritten important things, such as
        ...
        def overlay_cm(args):
            ...
            if cache and tree_key is not None:
                ...
                def populate(key, path):
                    ...
                    with _overlay_folders([base_path], upper=path, backend=overlay_backend) as path:
                ...
                cache_path = dir_cache.get_entry(key)
                with _overlay_folders([base_path, cache_path], backend=overlay_backend) as path:
                    yield dict(
                        path=path,
                        checksum=dir_cache.get_key_token(key),
                    )
            else:
                with _overlay_folders([base_path], backend=overlay_backend) as path:
                    prepare_overlay(path)
                    yield dict(
                        path=path,
                        checksum=None,
        ...
        if isinstance(tree, KernelTree):
            ...
            tree = KernelTree.from_target(
                ...
                **{
                    ...
                    if k not in {'path', 'checksum'}
                },
            )
            # This allows Target.get_kmod() to feedback an existing checksum so
            # we can hit the .ko cache without having to setup a tree when it
            # has already been done once.
            tree._update_spec(spec)

        arch = self.target.plat_info['abi']
