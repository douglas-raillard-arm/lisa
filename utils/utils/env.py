...
from utils.conf import JsonConf
from utils.platforms.juno_r0_energy import juno_r0_energy
from utils.platforms.hikey_energy import hikey_energy
from utils.platforms.pixel_energy import pixel_energy
...
class TestEnv(object):
    ...
    def ftrace_conf(self, conf):
        """
        Setup an Ftrace instrument

        :param conf: The Ftrace configuration
        :type conf: dict(dict(str))
        """
        # Merge with setup from target config
        target_conf = self.conf.get('ftrace', {})

        for item in ['events', 'functions']:
            conf[item] = sorted(
                set(target_conf.get(item, [])) |
                set(conf.get(item, []))
        )

        conf['buffsize'] = max(
            conf.get('buffsize', 0),
            target_conf.get('buffsize', FTRACE_BUFSIZE_DEFAULT)
        )
    ...
    @contextlib.contextmanager
    def record_ftrace(self, output_file=None, conf=None):

        if conf:
            self.ftrace_conf(conf)
