...
class Executor():
    ...
    def __init__(self, test_env, experiments_conf):
        ...
        if any(wl['type'] == 'rt-app'
               for wl in list(self._experiments_conf['wloads'].values())):
...
################################################################################
    ...
    def _setup_cpufreq(self, tc):
        ...
        if 'freqs' in cpufreq:
            ...
            for cpu, freq in cpufreq['freqs'].items():
    ...
    def _setup_files(self, tc):
        if 'files' not in tc:
            ...
            return True
        for name, value in tc['files'].items():
            check = False
            if name.startswith('!/'):
                check = True
                name = name[1:]
...
################################################################################
    ...
    def _wload_task_idxs(self, wl_idx, tasks):
        if type(tasks) == int:
            return list(range(tasks))
        if tasks == 'cpus':
            return list(range(len(self.target.core_names)))
        if tasks == 'little':
    ...
    def _wload_rtapp(self, wl_idx, wlspec, cpus):
        ...
        if conf['class'] == 'profile':
            ...
            for task_name, task in list(conf['params'].items()):
