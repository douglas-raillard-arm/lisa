...
class Target(object):
    ...
    def __init__(self,
        ...
        # For now we don't know how many connections are allowed on that
        # target, so play it safe and do not execute any command concurrently.
        self._bg_async_sem = asyncio.BoundedSemaphore(0)
        # Use 1 thread since the API does not allow 0 so that we still have an
        # object of the right type to simply paths using it, but the semaphore
        # will prevent any use of it.
        self._bg_async_pool = ThreadPoolExecutor(1)
    ...
    def _detect_max_async(self, max_async):
        ...
        # Aggressively attempt to create all the connections in parallel, so
        # that this setup step does not take too much time.
        ...
        self._bg_async_pool = ThreadPoolExecutor(max_conns)
        self._bg_async_sem = asyncio.BoundedSemaphore(max_conns)
    ...
    def disconnect(self):
        ...
        for conn in itertools.chain(connections, self._unused_conns):
            conn.close()
        self._bg_async_pool.__exit__(None, None, None)
    ...
    async def _execute_async(self, command, timeout=None, as_root=False, strip_colors=True, will_succeed=False, check_exit_code=True, force_locale='C'):
        sem = self._bg_async_sem
        pool = self._bg_async_pool
        ...
        try:
            ...
            # If there is just one asyncio task, doing an actual non-blocking
            # execute() is not useful since the event loop will not do anything
            # else while we wait for the command output anyway. OTH, we would
            # pay for some unnecessary overhead
        ...
        if blocking or sem.locked():
            ...
            # Do not use async context manager here so we can release the
            # semaphore early. This is important as asyncio.gather() has a
            # simple round-robin scheduling and tries to run each coroutine
            # once, then goes over the list again etc. The semaphore therefore
            # needs to be released in the same step as the result is ready.
            # Failure to do so leads to unnecessary starvation of the
            # semaphore, reducing the number of commands that can be ran in
            # parallel.
            await sem.acquire()
            def thread_f(loop, future):
                # Create the command from the thread, so that the
                # calling thread can await as quickly as possible to yield
                # to another task.
                excep = None
                try:
                    # Since we are in another thread, self.conn will be a
                    # separate connection instance than the one used for the
                    # main thread.
                    res = self._execute(
                        command,
                        timeout=timeout,
            ...
            pool.submit(thread_f, asyncio.get_running_loop(), async_future)
