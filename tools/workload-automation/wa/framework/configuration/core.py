...
class LoggingConfig(dict):
    ...
    def __init__(self, config=None):
        ...
        if isinstance(config, dict):
            config = {identifier(k.lower()): v for k, v in list(config.items())}
...
class JobSpec(Configuration):
    ...
    def merge_workload_parameters(self, plugin_cache):
        ...
        for source in self._sources:
            ...
            for name, cfg_point in list(cfg_points.items()):
                if name in config:
    ...
    def merge_runtime_parameters(self, plugin_cache, target_manager):
        ...
        for source, values in list(self.to_merge['runtime_parameters'].items()):
