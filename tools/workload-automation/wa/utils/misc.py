...
def get_random_string(length):
    ...
    return ''.join(random.choice(string.ascii_letters + string.digits) for _ in list(range(length)))
...
def istextfile(fileobj, blocksize=512):
    ...
    """
    _text_characters = (b''.join(chr(i) for i in list(range(32, 127))) +
                        b'\n\r\t\f\b')
...
def merge_maps(m1, m2):
    return type(m2)(chain(iter(list(m1.items())), iter(list(m2.items()))))


def merge_dicts_simple(base, other):
    result = base.copy()
    for key, value in list((other or {}).items()):
